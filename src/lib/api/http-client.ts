...
export function createHttpPublicClient(base = "/api/axs"): PublicClient {
  return {
    ...
    async getConfig(): Promise<PublicConfigView> { const config = await responseData<PublicConfigResponse["data"]>(await request(base, "/v1/public/config")); return { slots: config.slots.map(blockView), addons: (config.addons ?? []).map((addon) => ({ ...addon, currency: "MYR" as const })), onlinePayment: config.onlinePayment }; },
    async getBlocks() { return (await this.getConfig()).slots; },
    async validateVoucher(input: VoucherValidationRequest): Promise<VoucherValidation | null> { const response = await request(base, "/v1/public/vouchers/validate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input) }); if (response.status === 422) return null; return responseData<VoucherValidation>(response); },
