...
export const TabsList = React.forwardRef<React.ElementRef<typeof TabsPrimitive.List>, React.ComponentPropsWithoutRef<typeof TabsPrimitive.List>>(
  ({ className, ...props }, ref) => (
    <TabsPrimitive.List
      ref={ref}
      className={cn("inline-flex items-center gap-6 border-b border-[var(--line)]", className)}
      {...props}
    />
  ),
);
...
export const TabsTrigger = React.forwardRef<React.ElementRef<typeof TabsPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof TabsPrimitive.Trigger>>(
  ({ className, ...props }, ref) => (
    <TabsPrimitive.Trigger
      ref={ref}
      className={cn(
        "relative -mb-px border-b-2 border-transparent pb-3 text-sm font-bold text-[var(--muted)] transition-colors data-[state=active]:border-[var(--ink)] data-[state=active]:text-[var(--ink)]",
        className,
      )}
      {...props}
    />
