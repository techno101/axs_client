...
The Client sends no account ID. Its booking/order BFF rules forward only the opaque Customer cookie, letting Admin derive ownership. Public finder accepts a legacy or new AXS reference only; its masked response and short-lived grant stay at the same origin. Guest PDF requests send that grant only to the allowlisted public download route. `/api/customer/bookings`, `/[reference]`, `/download`, and `/[reference]/reschedule` are owner-only BFF rules and forward the Customer cookie server-side; the browser never receives an Admin origin, database path, session token, or provider secret.

The Client pins Admin v1.15 contract checksum `fb56a46557d58a0709761d5dbe5ddb7059d6f907a34b77c605fdf2d690bc6341` for the additive lookup/history/download/reschedule shapes.
...
`axs_admin` owns v1.15.0 OpenAPI. The copied static contract artifact is `src/lib/api/contract/v1.ts`, checksum `fb56a46557d58a0709761d5dbe5ddb7059d6f907a34b77c605fdf2d690bc6341`. `npm run contract:check` verifies the local representation; the client never imports admin source. Customer identity types grant no browser authority beyond the Client BFF.
